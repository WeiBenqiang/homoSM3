...
Version: SM3_V1.1
Date: Sep 18,2016
Description: This headfile provide macro defination, parameter definition and function declaration needed in SM3 algorithm implement
Function List: 
...
big-endian.
        //if CPU uses little-endian, BigEndian function is a necessary call to change the //little-endian format into big-endian format.
...
Input:        Bi[16]  //a block of a message null
Output:       W[64]
Return: 
Others:
...
/******************************************************************************
Function:
SM3_init         Description: initiate SM3 state
Calls:
Called By:      SM3_256
Input:          SM3_STATE *md
Output:         SM3_STATE *md
...
void SM3_process(SM3_STATE *md, unsigned char *buf, int len)
{
    while (len--)
    {
        ...
        printf("lala: %x ", md->buf[md->curlen]);
        md->curlen++;
...
void SM3_done(SM3_STATE *md, unsigned char hash[])
{
    ...
    md->length += md->curlen << 3; //msgLen* 8
    printf("md->length  : %d\n", md->length);
    /* append the '1' bit */
...
Input:      unsigned char buf[len]    //the input message
            int len
Output:         unsigned char hash[32]    //bytelen of the message
Return:     null
Others:
*******************************************************************************/
void SM3_256(unsigned char buf[], int len, unsigned char hash[])
{
    SM3_STATE md;
    SM3_init(&md);
    SM3_process(&md, buf, len); //len是buf的长度3
    SM3_done(&md, hash);
}
/******************************************************************************
...
Output:      null
Return:
            0        //the SM3 operation is correct
